...
object ModelRepository {
    ...
    suspend fun initialize(): ModelMarketConfig? = initializationMutex.withLock {
        val existingConfig = _marketDataFlow.value
        if (existingConfig != null) {
            val currentState = _loadingStateFlow.value
            Log.i(TAG, "initialize reused existing config: currentState=$currentState version=${existingConfig.version}")
            return@withLock existingConfig
        }
    ...
    suspend fun forceRefresh() {
        if (_loadingStateFlow.value == LoadingState.IDLE || _marketDataFlow.value == null) {
            initialize()
            return
        }
    ...
    suspend fun getMarketDataSuspend(): ModelMarketConfig {
        _marketDataFlow.value?.let { return it }
        initialize()?.let { return it }
        loadCachedOrAssets()?.let { fallback ->
            _marketDataFlow.value = fallback
            cachedModelMarketData = fallback
            _loadingStateFlow.value = LoadingState.LOADED
            _errorFlow.value = null
            buildCacheFromData(fallback)
            TagMapper.initializeFromConfig(fallback)
            return fallback
        }
        throw IllegalStateException("Model market data is unavailable")
    }
