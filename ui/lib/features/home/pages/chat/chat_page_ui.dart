...
mixin _ChatPageUiMixin on _ChatPageStateBase {
  ...
  Widget _buildModeMessagePage(
    ...
    return ChatMessageList(
      ...
      editingUserMessageId: mode == ChatPageMode.normal
          ? _editingUserMessageId
          : null,
      userMessageEditController: _userMessageEditController,
      userMessageEditFocusNode: _userMessageEditFocusNode,
      isSubmittingUserMessageEdit: _isSubmittingUserMessageEdit,
      onCancelUserMessageEdit: _cancelEditingUserMessage,
      onSubmitUserMessageEdit: mode == ChatPageMode.normal
  ...
  Future<void> _handleUserMessageLongPressStart(
    ...
    switch (action) {
      ...
      case _UserMessageQuickAction.edit:
        _startEditingUserMessage(message);
        return;
  ...
  Future<_UserMessageQuickAction?> _showUserMessageQuickMenu(
    ...
    final estimatedMenuHeight =
        60.0 + (showEditAction ? 56.0 : 0.0) + (showRetryAction ? 56.0 : 0.0);
  ...
  bool _canEditUserMessage(ChatMessageModel message) {
    return _canRetryUserMessage(message);
  }
  ...
  void _startEditingUserMessage(ChatMessageModel message) {
    ...
    setState(() {
      ...
      _isSubmittingUserMessageEdit = false;
      _userMessageEditController.value = TextEditingValue(
        text: text,
        selection: TextSelection.collapsed(offset: text.length),
      );
    });
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted || _editingUserMessageId != message.id) {
        return;
      }
      _userMessageEditFocusNode.requestFocus();
    });
  }

  void _cancelEditingUserMessage() {
    if (_editingUserMessageId == null && !_isSubmittingUserMessageEdit) {
      return;
    }
    _userMessageEditFocusNode.unfocus();
    setState(() {
  ...
  Future<void> _submitEditedUserMessage(ChatMessageModel message) async {
    if (_isSubmittingUserMessageEdit) {
      return;
    }
    if (!_canEditUserMessage(message)) {
      showToast(
    ...
    final editedText = _userMessageEditController.text.trim();
    ...
    setState(() {
      _isSubmittingUserMessageEdit = true;
    });
    try {
      ...
      showToast(
        LegacyTextLocalizer.isEnglish
            ? 'Failed to resend edited message'
            : '发送编辑后的消息失败',
